...
def __login(username, password):
    ...
    request_basic.__APNToken = None #result.json()['Data']['Token']
    request_basic.__cookies=result.cookies
    return result.json()
...
# 获取当前正在执行的任务数据
def __get_task():
    result = request_basic.__get(__server_url + "/Task/GetSimTaskByUsid?onlySimTask=true", None)
    task = result.json()
    if task['RespCode'] == 1:
        # 设置当前任务Token
...
def __get_image(objectId):
    result = request_node.__get("/Data/" + str(objectId), None)
