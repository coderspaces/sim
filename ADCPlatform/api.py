...
# 控制车辆
def control(throttle, steering, brake, gear=1):
    # business.__send_command('Throttle', '1')
    business.__send_command('control', str(business.simtask['Sences']['Vehicles'][0]['ID']) +'/' + str(throttle) + '/' + str(steering) + '/' + str(brake))
    # business.__send_command('control', str(business.simtask['Sences']['Vehicles'][0]['ID']) +'/' + str(throttle) + '/' + str(steering) + '/' + str(brake)+ '/' + str(gear))
